...
const LearningNow: React.FC = () => {
    ...
    const savedTexts = useAppSelector((state: RootState) => state.savedLearningNow.items);
    
    const [editText, setEditText] = useState<{ id: number; text: string } | null>(null); 

    // Save or remove text based on its existence
    ...
    // Update the text in the savedLearningNow state
    const handleUpdate = () => {
        if (editText) {
            console.log("Güncelleme işlemi için:", editText); // Burada güncellenmeye çalışılan metni görebilirsiniz
            dispatch(updateText(editText)); 
            setEditText(null);
        }
    };

    // Available words with their translations
    const words = [
                ...
                <table>
                    <tbody>
                        {words.map(({ id, word, translation }) => (
                            <tr className='list_learing' key={id}>
                                <td className='list_left'>{word}</td>
                                <td>{translation}</td>
                                <td>
                                    <svg onClick={() => handleSave(id, word)} width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <button onClick={() => handleEdit(id, word)}>
                                        Edit
                                    </button>
                                </td>
                            </tr>
