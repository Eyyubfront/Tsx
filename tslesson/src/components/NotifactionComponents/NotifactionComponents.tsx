...
const NotifactionComponents = () => {
    ...
    notifications.forEach(notification => {
        const formattedDate = formatDate(notification.createdOn);
        if (!groupedNotifications.has(formattedDate)) {
            groupedNotifications.set(formattedDate, []);
        }
        groupedNotifications.get(formattedDate).push(notification);
    });
            ...
            <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={handleClose}>
                ...
                {[...groupedNotifications.entries()].map(([date, items]) => (
                    <div key={date} className="notification-group">
                        <div className='form_datetop'>{date}</div>
                        {items.map((notification: Notification) => (
                            <MenuItem key={notification.id} className='listhead' onClick={handleClose}>
                                <div className="message_box">
                                    ...
                                    <div>{formatTime(notification.createdOn)}</div>
                                </div>
                            </MenuItem>
                        ))}
                    </div>
